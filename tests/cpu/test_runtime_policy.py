...
def test_controller_uses_progress_over_total_cost_not_acceptance_alone():
    ...
    for b, progress, cost in [(1, 1, 5), (2, 2, 6), (4, 4, 7), (8, 8, 50), (16, 16, 100)]:
        c.observe(b, batch_size=1, context_len=1024, progress=progress,
                  draft_ms=cost / 2, verify_ms=cost / 2)
...
def test_adaptation_to_rejection_and_restore_cost():
    ...
    c.observe(16, batch_size=1, context_len=1024, progress=1, draft_ms=5, verify_ms=10, restore_ms=20)
...
def test_memory_pressure_removes_large_blocks_and_checkpoint():
    ...
    assert budget.feasible_blocks([1, 2, 4, 8, 16], live_bytes=200,
                                  bytes_per_block_token=40, checkpoint_bytes=100) == [1, 2, 4]
    assert budget.feasible_blocks([1, 2, 4], live_bytes=450,
                                  bytes_per_block_token=40, checkpoint_bytes=100) == [1]
