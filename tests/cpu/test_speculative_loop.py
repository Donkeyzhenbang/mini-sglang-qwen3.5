...
class ToyTarget:
    ...
    def checkpoint(self): return list(self.tokens)
...
def test_adaptive_loop_and_low_memory_fallback():
    prompt = [1, 3, 5]
    baseline = generate(ToyTarget(), None, prompt, 40, block_size=1)
    result = generate(ToyTarget('partial'), Draft(), prompt, 40, block_size=16,
        adaptive=AdaptiveBlockController(), feasible=lambda context: [1, 2, 4])
