...
def test_prefill_and_decode_share_one_gdn(monkeypatch):
    ...
    monkeypatch.setattr(attention, 'SUPPORTED_ATTENTION_BACKENDS',
...
def test_4b_memory_layout_and_admission():
    config = SimpleNamespace(full_attention_layer_ids=list(range(3, 32, 4)),
        num_layers=32, num_kv_heads=4, head_dim=256, linear_num_key_heads=16,
    ...
    args = dict(available_bytes=1 << 30, page_size=256, layout=layout, slots=5,
                workspace_bytes=128 << 20, snapshot_bytes=0)
    pages = plan_kv_pages(**args)
    assert (pages + 1) * 256 * layout.kv_bytes_per_token + 5 * layout.state_bytes_per_slot + (128 << 20) <= 1 << 30
