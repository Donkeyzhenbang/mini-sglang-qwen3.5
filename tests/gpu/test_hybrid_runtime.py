...
def test_packed_extend_matches_recurrent_oracle():
    ...
    actual = packed_extend(qkv, a, b, alog, dt, actual_state,
        torch.tensor(slots, dtype=torch.int32, device='cuda'),
        torch.tensor([0, 7, 9, 13], dtype=torch.int32, device='cuda'), h, hv, k, v)
    ...
    for slot, length in zip(slots, lengths):
        ...
        for i in range(offset, offset + length):
            ...
            q, key = q.view(h, k).repeat_interleave(hv//h, 0), key.view(h, k).repeat_interleave(hv//h, 0)
            ...
            delta = (value.view(hv, v) - torch.einsum('hvk,hk->hv', state, key)) * b[i].sigmoid()[:, None]
...
def test_gpu_eviction_offloads_complete_bundle():
    ...
    payload = {'kv': torch.arange(4, dtype=torch.float32, device='cuda'),
