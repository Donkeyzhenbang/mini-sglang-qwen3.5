...
for length in map(int, args.lengths.split(',')):
    ...
    for i in range(args.requests_per_length):
        ...
        ids = prefix if i % 4 == 0 else prefix + [rng.randrange(100, 10000) for _ in range(length - len(prefix))]
        rows.append(dict(input_ids=ids, max_new_tokens=args.output_tokens,
