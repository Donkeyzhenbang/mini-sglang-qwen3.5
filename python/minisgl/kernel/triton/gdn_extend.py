...
@triton.jit
def _extend_kernel(QKV, A, B, ALOG, DT, STATE, SLOTS, CU, OUT,
...
def packed_extend(mixed_qkv, a, b, A_log, dt_bias, state, state_indices, cu_seqlens,
    ...
    out = torch.empty((len(mixed_qkv), num_v_heads, head_v_dim),
                      device=mixed_qkv.device, dtype=mixed_qkv.dtype)
    bv = min(triton.next_power_of_2(head_v_dim), 32)
    _extend_kernel[(triton.cdiv(head_v_dim, bv), len(state_indices) * num_v_heads)](
        mixed_qkv, a, b, A_log, dt_bias, state, state_indices, cu_seqlens, out,
        D=mixed_qkv.shape[1], H=num_q_heads, HV=num_v_heads, K=head_k_dim, V=head_v_dim,
        BK=triton.next_power_of_2(head_k_dim), BV=bv, num_warps=1)
    return out
