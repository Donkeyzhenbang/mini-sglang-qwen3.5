...
class AdaptiveBlockController:
    ...
    def choose(
        self, *, batch_size: int, context_len: int, feasible_blocks=None, remaining: int = 1 << 30
    ) -> int:
        ...
        return max(
            blocks,
            key=lambda b: (
                self.observations[(*bucket, b)].progress / self.observations[(*bucket, b)].cost_ms
            ),
        )
...
class JointMemoryBudget:
    ...
    def feasible_blocks(
        self, blocks, *, live_bytes: int, bytes_per_block_token: int, checkpoint_bytes: int = 0
    ) -> list[int]:
        if (
            min(
                self.capacity_bytes,
        ...
        return [
            b
            for b in blocks
            if b * bytes_per_block_token + (checkpoint_bytes if b > 1 else 0) <= free
        ]
