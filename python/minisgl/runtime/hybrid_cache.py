...
class HybridPrefixCache:
    def __init__(self, gpu_bytes: int, host_bytes: int, *, policy: str = 'cost',
        ...
        self.stats = dict(hits=0, misses=0, offloads=0, evictions=0,
                          recompute_choices=0, transfer_ms=0.0)
    ...
    def _make_room(self, tier: str, needed: int) -> bool:
        ...
        while self.used(tier) + needed > self.budgets[tier]:
            ...
            if (tier == 'gpu' and victim.recompute_ms > transfer_ms
    ...
    def lookup(self, tokens) -> CacheEntry | None:
        key = tuple(int(t) for t in tokens)
        candidates = [e for p, e in self.entries.items() if len(p) <= len(key) and key[:len(p)] == p]
