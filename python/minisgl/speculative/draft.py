...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        y = x.float()
        return (y * torch.rsqrt(y.square().mean(-1, keepdim=True) + self.eps)).to(x.dtype) * self.weight
...
class DraftAttention(nn.Module):
    def __init__(self, c, layer_id):
        ...
        sliding = c.get('layer_types', ['full_attention'] * c['num_hidden_layers'])[layer_id] == 'sliding_attention'
...
class DFlashDraft(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        if config.get('architectures') != ['DFlashDraftModel'] or config.get('model_type') != 'qwen3':
